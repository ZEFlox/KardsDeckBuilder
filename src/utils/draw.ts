...
export async function combineImages(deck: deck, name: string): Promise<void> {
  // 1. 获取所有图片
  ...
  ctx.drawImage(
    bg,
    (width - bg.width * 1.5) / 2,
    (height - bg.height * 1.5) / 2,
    bg.width * 1.5,
    bg.height * 1.5,
  )
  ...
  const bc = await new Promise<HTMLImageElement>((resolve, reject) => {
    const img = new Image()
    img.src = './back.png'
    img.onload = () => resolve(img)
    img.onerror = (e) => reject(`图片加载失败: ${e}`)
